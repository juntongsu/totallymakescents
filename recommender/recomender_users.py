...
def has_opinions(user, frame):
    return set(frame.loc[(frame['User ID'] == user)]["Perfume ID"])
...
def sentiment_vector(user, frame, perfumes = "ALL"):
    if perfumes == "ALL":
        sentiment = frame.loc[(frame["User ID"] == user)][["Perfume ID","Sentiment"]]
    else:
        sentiment = frame.loc[((frame["User ID"] == user) & (frame["Perfume ID"].isin(perfumes)))][["Perfume ID","Sentiment"]]
    
    sentiment = sentiment.sort_values(by="Perfume ID")
...
def ratings(user_1,frame_1,frame_2, commonality=1,weight=1.0):
    ...
    adjusted_ratings = adjusted_ratings.loc[(~adjusted_ratings["Perfume ID"].isin(known_perfumes))]

    adjusted_ratings["Similarity"] = adjusted_ratings.groupby('Perfume ID')['Sentiment'].transform('sum')
    adjusted_ratings = adjusted_ratings[["Perfume ID", "Perfume Name", "Similarity"]].drop_duplicates()
