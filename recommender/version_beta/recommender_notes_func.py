def find_overlap(df_persian, df_data_notes, language):
    ...
    if language == 0:
        ...
        perf_name_data_notes = df_fra['Perfume'].convert_dtypes()
        perf_url_data_notes = df_fra['url'].convert_dtypes()
        perf_rate_data_notes = df_fra['Rating Count'].convert_dtypes()
        
    elif language == 1:
        perf_name_unique = df_persian['Perfume Name'].drop_duplicates().convert_dtypes().to_list()
        perf_name_original = perf_name_unique
        perf_name_data_notes = df_aromo['name'].convert_dtypes()
        perf_url_data_notes = df_aromo['url'].convert_dtypes()
        perf_rate_data_notes = df_aromo['rating_count'].fillna('0 c').str.split(' ').str[0].astype('int')
...
def notes_vec_prep(perf_overlap, language):
    if language == 0:
        df_perf_notes = df_fra[(df_fra['url'].isin(perf_overlap['url']))][['Perfume', 'Top', 'Middle', 'Base']]
        ...
        df_perf_notes = df_aromo[df_aromo['url'].isin(perf_overlap['url'])][['name', 'top_notes', 'middle_notes', 'bottom_notes']]
...
def input_perfume():
    ...
    df_input = pd.read_csv('{}input.csv'.format(path_data), header=None)
...
def user_vec_prep(user_perfume, user_sentiment):
    for i in range(len(user_sentiment)):
        if i == 0:
...
def recommender_notes(perf_names, vec_top, vec_mid, vec_base, user_top, user_mid, user_base, temperature):
